...
const commentsFetcher = async (endPoint: string) =>
...
export default function WorkoutScreenCustomer(): JSX.Element {
  ...
  } = useSWR<Comment[]>(`${APIRoute.Reviews}/${workoutId}`, commentsFetcher);
  ...
  return commentsIsLoading ? (
    ...
    <Error
      errorMessage={
        commentsError.message || commentsError.response.data.message
      }
    />
  ) : (
                  ...
                  onClick={async (evt) => {
                    if (evt.currentTarget.textContent === 'Приступить') {
                      try {
                        ...
                        buttonStartRef.current?.classList.add(
                          'show-more__button--error'
                        );

                        setTimeout(
                          () =>
                            buttonStartRef.current?.classList.remove(
                              'show-more__button--error'
                            ),
                          SHOW_ERROR_TIME
                        );
