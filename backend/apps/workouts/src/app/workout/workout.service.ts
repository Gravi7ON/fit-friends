...
import { RANDOM_STATIC_IMAGE_PATH, SortFiled, SortingDirection, WorkoutMessageException } from './workout.constant';
...
export class WorkoutService {
  ...
  async findCoachOrders(coachId: string, query: CoachOrdersQuery): Promise<Workout[]> {
    ...
    const orders = await this.workoutRepository.findOrders(coachId, existedWorkoutId, query);

    const transformOrders = orders
      .map((workout) => {
        return {
          ...workout,
          orders: workout.orders
            .reduce((prev: [{sum: number; amountWorkout: number}], order: OrderWorkout) => ([{
    ...
    switch(query.sortField) {
      case SortFiled.Sum:
        if (query.sortDirection === SortingDirection.Asc) {
          return transformOrders.sort((a, b) => a.orders[0].sum - b.orders[0].sum);
        } else {
          return transformOrders.sort((a, b) => b.orders[0].sum - a.orders[0].sum);
        }
      case SortFiled.AmountWorkout:
        if (query.sortDirection === SortingDirection.Asc) {
          return transformOrders.sort((a, b) => a.orders[0].amountWorkout - b.orders[0].amountWorkout);
        } else {
          return transformOrders.sort((a, b) => b.orders[0].amountWorkout - a.orders[0].amountWorkout);
        }
